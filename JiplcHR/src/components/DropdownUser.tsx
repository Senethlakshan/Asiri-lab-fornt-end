...
import adminImage from '../images/user/personal-security.png';
import managerImage from '../images/user/group.png';
import userImage from '../images/user/userCompany.png';

import { useSelector, useDispatch } from 'react-redux';
import { RootState } from '../routes/store';
import { clearUserID } from '../routes/userSlice';
...
const DropdownUser = () => {
  ...
  //         const response = await fetch(`http://localhost:5000/api/auth/user/userbyId/${userID}`);
  //          console.log(userID); 
    ...
    const fetchUserData = async () => {
      ...
      try {
        const response = await fetch(
          `http://localhost:5000/api/auth/user/userbyId/${userID}`,
        );
        if (!response.ok) throw new Error('Failed to fetch user data');
        const data = await response.json();
  ...
  return (
    <div className="relative">
      <Link
        ...
        to="#"
      >
        {/* show user name and designation */}
        <span className="hidden text-right lg:block">
          <span className="block text-xl  font-medium text-black dark:text-white">
            {userData.username}
          </span>
          <span className="block test-m">{userData.designation}</span>
          {/* <span className="block text-xs">Role : Manager</span> */}
        </span>
      ...
      >
        ...
        <button
          className="flex items-center gap-3.5 py-4 px-6 text-sm font-medium 
        duration-300 ease-in-out hover:text-black lg:text-base  cursor-pointer"
          ...
          <div className=" flex items-center justify-center">
            ...
            <div className="relative bg-white p-6 rounded-md shadow-md w-96">
              ...
              <div className="flex justify-end mt-4">
                <button
                  className="px-4 py-2 mr-2 bg-red text-white cursor-pointer rounded-md Logout-cancel-button-dashbord "
                  onClick={handleCancelLogout}
                >
                  Cancel
                </button>
                <button
                  className="px-4 py-2 bg-yellow text-white  rounded-md Logout-button-dashbord"
      ...
      <ToastContainer
        ...
        autoClose={2000}
        hideProgressBar
        className="logout-toast-container"
      />
